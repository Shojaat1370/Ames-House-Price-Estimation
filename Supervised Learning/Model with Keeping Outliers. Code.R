...
# First Scenario :  Analsis with keeping outliers 
...
# creating sub-dataset for these variable in order to combine with previous "original_cat_dataset":
...
find_outlier(numeric_data$BsmtFinSF2)
length(find_outlier(numeric_data$BsmtFinSF2))

# ---------        7.2.   Second Step: Plotting   -----------

# using 'Scatter' plot for numerical values in order to display the shape of some important features 
# in order to identify existence of any outliers and non-linear relationship.

library(ggplot2) 

# Ploting the "Scatter Plot" for "LotFrontage", also Adding a line using the lm method (linear regression)
options(repr.plot.width=5, repr.plot.height=4)
ggplot(numeric_data, aes(x = numeric_data$LotFrontage , y = SalePrice)) +
  geom_point() +
...
options(repr.plot.width=5, repr.plot.height=4)
ggplot(numeric_data, aes(x = numeric_data$LotArea , y = SalePrice)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  
...
options(repr.plot.width=5, repr.plot.height=4)
ggplot(numeric_data, aes(x = numeric_data$GarageArea , y = SalePrice)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  
...
options(repr.plot.width=5, repr.plot.height=4)
ggplot(numeric_data, aes(x = numeric_data$GrLivArea , y = SalePrice)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  
...
# in this case, I do not handle outliers in order to work with algorithms without removing outliers. 
...
#    9.2.3. Checking number of outliers for some features such as "LotFrontage". 'After' scaling

num_out_LotF2 <- find_outlier(numeric_data$LotFrontage)
num_out_LotF2
length(num_out_LotF2)       # result 47

num_out_LotArea2 <- find_outlier(numeric_data$LotArea)
num_out_LotArea2
length(num_out_LotArea2)     # result 33

num_out_GarageArea2 <- find_outlier(numeric_data$GarageArea)
num_out_GarageArea2
length(num_out_GarageArea2)   # result 7

# Result shows that scaling did not effect the number of outliers for these features, so
# scaling does not solve outliers problem in this dataset.


#    9.2.4. Checking Normality again after scaling 
...
# -----------     9.3    Checking again and deleting most skewed data after scaling:    -----

# In this step some features that have more skewness even after scaling are be deleted.
# When the skewness is more than 1 then we can say data is heavily skewed. So  
...
plot(numeric_data$BsmtFinSF2, SalePrice, type = "p", main = "Scatter Plot", xlab = "BsmtFinSF2", ylab = "SalePrice")
plot(numeric_data$LowQualFinSF, SalePrice, type = "p", main = "Scatter Plot", xlab = "LowQualFinSF", ylab = "SalePrice")
plot(numeric_data$PoolArea, SalePrice, type = "p", main = "Scatter Plot", xlab = "PoolArea", ylab = "SalePrice")
plot(numeric_data$MiscVal, SalePrice, type = "p", main = "Scatter Plot", xlab = "MiscVal", ylab = "SalePrice")
...
#  ////////////////  10. Correlation between target variable and numerical data ///////////// 
...
#  optimal lambda value based on cross-validation that minimizes test MSE
...
svm_model_poly <- svm(train_labels ~ ., data = train_data, kernel = "radial")
